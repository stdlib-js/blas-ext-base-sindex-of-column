...
* // returns 1
*/
function sindexOfColumn( order, M, N, A, LDA, x, strideX, workspace, strideW ) {
	var sa1;
	var sa2;
	var s;
	var l;
	...
	if ( l === null ) {
		throw new TypeError( format( '2iRFx', order ) );
	}
	if ( isRowMajor( l ) ) {
		s = N;
		sa1 = LDA;
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '2iRIR', max( 1, s ), LDA ) );
	}
